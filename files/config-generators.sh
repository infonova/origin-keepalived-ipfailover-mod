...
"
  ...
  while [[ "${vip_counter}" -lt "${#total_vips[@]}" ]]; do
    ...
    # if openshift_ha_node_id is set ...
    if [[ ! -z "${OPENSHIFT_HA_NODE_ID}" ]]; then
      local nodeid=$(echo "${OPENSHIFT_HA_NODE_ID}" | rev | cut -d'-' -f1 | rev)
      # ... override priority in case nodeid == counter
      if [[ ${counter} -eq $((nodeid + 1)) ]]; then
        priority=255
      fi
    fi
